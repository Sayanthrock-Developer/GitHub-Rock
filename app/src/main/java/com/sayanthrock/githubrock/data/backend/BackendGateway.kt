...
import com.sayanthrock.githubrock.core.network.BackendWebOAuthExchangeRequest
...
class BackendEndpointStore @Inject constructor(
    ...
    fun save(rawEndpoint: String): String {
        val endpoint = requireNotNull(normalizedBackendBaseUrl(rawEndpoint)) { "Enter the HTTPS URL of the deployed GitHub Rock Backend." }
...
class BackendGateway @Inject constructor(
    ...
    suspend fun pollDeviceFlow(deviceCode: String): DeviceTokenResponse =
        apiForConfiguredEndpoint().pollDeviceFlow(BackendDevicePollRequest(deviceCode)).toDeviceTokenResponse()

    suspend fun refreshToken(refreshToken: String): DeviceTokenResponse {
        val api = apiForConfiguredEndpoint()
        validateBackendForApp(api.config(), "oauthRefreshProxy")
        return api.refreshToken(BackendTokenRefreshRequest(refreshToken)).toDeviceTokenResponse()
    }

    suspend fun startWebOAuth(state: String): String {
        val endpoint = requireNotNull(endpointStore.endpoint()) { "GitHub Rock Backend is not connected." }
        val config = api(endpoint).config()
        validateBackendForApp(config, "oauthWeb")
        return "${endpoint.trimEnd('/')}/v1/auth/github/start?state=${java.net.URLEncoder.encode(state, Charsets.UTF_8.name())}"
    }

    suspend fun exchangeWebOAuth(code: String): DeviceTokenResponse {
        val api = apiForConfiguredEndpoint()
        validateBackendForApp(api.config(), "oauthWeb")
        return api.exchangeWebOAuth(BackendWebOAuthExchangeRequest(code)).toDeviceTokenResponse()
    }

    private fun apiForConfiguredEndpoint(): GitHubRockBackendApi =
        api(requireNotNull(endpointStore.endpoint()) { "GitHub Rock Backend is not connected." })
...
internal fun requireBackendBaseUrl(raw: String): String = requireNotNull(normalizedBackendBaseUrl(raw)) {
    "Use a valid HTTPS backend URL without query parameters or credentials."
}

internal fun validateBackendForApp(config: BackendPublicConfigResponse, requiredFeature: String, currentVersion: String = BuildConfig.VERSION_NAME) {
    require(config.apiVersion == "v1") { "Backend API ${config.apiVersion} is not compatible with this app." }
    require(!config.maintenanceMode) { "GitHub Rock Backend is temporarily in maintenance mode." }
    require(isVersionAtLeast(currentVersion, config.minSupportedAppVersion)) { "GitHub Rock ${config.minSupportedAppVersion} or newer is required by the backend." }
    require(config.features[requiredFeature] == true) { "Backend feature $requiredFeature is not available." }
}
...
internal fun BackendDeviceTokenResponse.toDeviceTokenResponse(): DeviceTokenResponse = when (state) {
    "authorized" -> DeviceTokenResponse(accessToken, tokenType, scope, expiresIn, refreshToken, refreshTokenExpiresIn)
    "pending" -> DeviceTokenResponse(error = "authorization_pending", errorDescription = message)
    "slow_down" -> DeviceTokenResponse(error = "slow_down", errorDescription = message)
    "expired" -> DeviceTokenResponse(error = "expired_token", errorDescription = message)
    "denied" -> DeviceTokenResponse(error = "access_denied", errorDescription = message)
    else -> DeviceTokenResponse(error = "backend_error", errorDescription = message ?: "GitHub Rock Backend could not complete authentication.")
}
