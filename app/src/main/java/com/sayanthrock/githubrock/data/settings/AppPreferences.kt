...
enum class LoadingStyle { Spinner, Linear, Pulse, Skeleton, Liquid, Orbit, Shimmer, Morph, RockRing; companion object { fun fromStored(value: String?): LoadingStyle = entries.firstOrNull { it.name == value } ?: Spinner } }
enum class AnimationStyle { Liquid, Spring, Cinematic, Magnetic, Dynamic, RockFlow; companion object { fun fromStored(value: String?): AnimationStyle = entries.firstOrNull { it.name == value } ?: Spring } }
...
class AppPreferences @Inject constructor(@ApplicationContext private val context: Context) {
    val appearance: Flow<AppearancePreferences> = context.dataStore.data.map { preferences ->
        AppearancePreferences(
            ...
            recentAccentColors = parseRecentAccentColors(preferences[RECENT_ACCENT_COLORS]),
    ...
    suspend fun setAppLanguage(tag: String?) = context.dataStore.edit { preferences ->
        if (tag.isNullOrBlank()) preferences.remove(APP_LANGUAGE_TAG) else preferences[APP_LANGUAGE_TAG] = tag
    }
    ...
    suspend fun setWhatsNewTranslationLanguage(language: String?) = context.dataStore.edit { preferences ->
        if (language.isNullOrBlank() || language == "en") preferences.remove(WHATS_NEW_TRANSLATION_LANGUAGE)
