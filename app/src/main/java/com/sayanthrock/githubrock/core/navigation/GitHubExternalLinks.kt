...
object GitHubUrlPolicy {
    ...
    fun isGitHubHttpsUrl(rawUrl: String): Boolean { val uri = runCatching { URI(rawUrl) }.getOrNull() ?: return false; return uri.scheme?.lowercase() == "https" && uri.host?.lowercase() in trustedHosts }
    fun isRepositoryUrl(rawUrl: String): Boolean { if (!isGitHubHttpsUrl(rawUrl)) return false; val uri = runCatching { URI(rawUrl) }.getOrNull() ?: return false; if (uri.host?.lowercase() !in repositoryHosts) return false; val segments = uri.path.orEmpty().split('/').filter(String::isNotBlank); if (segments.size < 2 || segments.first().lowercase() in reservedRoots) return false; return repositorySegment.matches(segments[0]) && repositorySegment.matches(segments[1]) }
}
...
object GitHubExternalLinkLauncher {
    ...
    /** Opens a backend OAuth URL only after the backend endpoint has already been validated as HTTPS. */
    fun openOAuthUrl(context: Context, rawUrl: String): Boolean {
        val uri = runCatching { URI(rawUrl) }.getOrNull() ?: return false
        if (!uri.scheme.equals("https", true) || uri.host.isNullOrBlank() || uri.userInfo != null || uri.fragment != null) return false
    ...
    private fun launchCustomTab(context: Context, rawUrl: String, browserPackage: String, ephemeral: Boolean): Boolean {
        val customTab = CustomTabsIntent.Builder().setShowTitle(true).setShareState(CustomTabsIntent.SHARE_STATE_OFF).apply { if (ephemeral) setEphemeralBrowsingEnabled(true) }.build().apply { intent.addCategory(Intent.CATEGORY_BROWSABLE); intent.setPackage(browserPackage); if (context !is Activity) intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK) }
    ...
    private fun launchBrowserIntent(context: Context, rawUrl: String): Boolean {
        val browserIntent = Intent(Intent.ACTION_VIEW, Uri.parse(rawUrl)).apply { addCategory(Intent.CATEGORY_BROWSABLE); selector = Intent(Intent.ACTION_VIEW, Uri.parse("https://")).apply { addCategory(Intent.CATEGORY_BROWSABLE) }; if (context !is Activity) addFlags(Intent.FLAG_ACTIVITY_NEW_TASK) }
