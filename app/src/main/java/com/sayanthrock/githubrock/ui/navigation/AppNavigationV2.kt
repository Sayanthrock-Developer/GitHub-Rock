...
fun MainNavigationV2(navController: NavHostController, state: MainUiState, onSearch: (com.sayanthrock.githubrock.core.model.RepositorySearchOptions) -> Unit, onInspectProfile: (String) -> Unit, onRememberRepository: (com.sayanthrock.githubrock.core.model.GitHubRepositoryModel) -> Unit, onOpenGitHubUrl: (String) -> Unit, onRefresh: () -> Unit, onLogout: () -> Unit) {
    ...
    Box(Modifier.fillMaxSize()) {
        NavHost(navController, TopDestinationV2.Home.route, Modifier.fillMaxSize().widthIn(max = 1200.dp)) {
            ...
            composable(TopDestinationV2.Builds.route) { BuildsScreen(mode, state.repositories, state.workflowRuns, openRepo, { repo, run -> navController.navigate("build-details/${repo.owner.login}/${repo.name}/${run.id}") }) }
            composable(BUILD_STATUS_ROUTE, arguments = listOf(navArgument("filter") { type = NavType.StringType })) { e -> BuildStatusPage(mode, state.repositories, state.workflowRuns, e.arguments?.getString("filter").orEmpty(), openRepo, { repo, run -> navController.navigate("build-details/${repo.owner.login}/${repo.name}/${run.id}") }) }
            ...
            composable(BUILD_JOB_DETAILS_ROUTE, arguments = listOf(navArgument("owner") { type = NavType.StringType }, navArgument("repo") { type = NavType.StringType }, navArgument("runId") { type = NavType.LongType }, navArgument("jobId") { type = NavType.LongType })) { e ->
                ...
                state.repositories.firstOrNull { it.owner.login.equals(owner, true) && it.name == repo }?.let { BuildJobDetailsScreen(mode, it, runId, jobId, navController::navigateUp) } ?: Text("Repository unavailable")
            }
            composable(BUILD_ARTIFACT_DETAILS_ROUTE, arguments = listOf(navArgument("owner") { type = NavType.StringType }, navArgument("repo") { type = NavType.StringType }, navArgument("runId") { type = NavType.LongType }, navArgument("artifactId") { type = NavType.LongType })) { e ->
                ...
                state.repositories.firstOrNull { it.owner.login.equals(owner, true) && it.name == repo }?.let { BuildArtifactDetailsScreen(it, runId, artifactId, navController::navigateUp) } ?: Text("Repository unavailable")
            }
            ...
            composable("build/{owner}/{repo}/{runId}", arguments = listOf(navArgument("owner") { type = NavType.StringType }, navArgument("repo") { type = NavType.StringType }, navArgument("runId") { type = NavType.LongType }), deepLinks = listOf(navDeepLink { uriPattern = "githubrock://build/{owner}/{repo}/{runId}" })) { e -> BuildsScreen(mode, state.repositories, state.workflowRuns, openRepo, initialRepository = state.repositories.firstOrNull { it.owner.login == e.arguments?.getString("owner") && it.name == e.arguments?.getString("repo") }, initialRunId = e.arguments?.getLong("runId")) }
