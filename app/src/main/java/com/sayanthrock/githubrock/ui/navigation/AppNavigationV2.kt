...
fun MainNavigationV2(
    ...
    val buildRepositories = buildList<com.sayanthrock.githubrock.core.model.GitHubRepositoryModel> {
        recentRepositories.forEach { recent -> if (none { it.id == recent.id }) add(recent) }
        ownRepositories.forEach { repo -> if (none { it.id == repo.id }) add(repo) }
    }
    ...
    val selectBuildRepository: (com.sayanthrock.githubrock.core.model.GitHubRepositoryModel) -> Unit = { repo -> onRememberRepository(repo) }
    ...
    Box(Modifier.fillMaxSize()) {
        NavHost(navController, TopDestinationV2.Home.route, Modifier.fillMaxSize().widthIn(max = 1200.dp)) {
            ...
            ) { HomeScreen(state.repositories, openRepo, state.isLoading, state.isRefreshing, onRefresh, mode == AppMode.Connected) }
            ...
            composable(BUILD_STATUS_ROUTE, arguments = listOf(navArgument("filter") { type = NavType.StringType })) { e ->
                BuildStatusPage(mode, buildRepositories, state.workflowRuns, e.arguments?.getString("filter").orEmpty(), openRepo,
                    { repo, run -> navController.navigate("build-details/${repo.owner.login}/${repo.name}/${run.id}") }, state.profile?.login)
            }
            composable(BUILD_DETAILS_ROUTE, arguments = listOf(navArgument("owner") { type = NavType.StringType }, navArgument("repo") { type = NavType.StringType }, navArgument("runId") { type = NavType.LongType })) { e ->
                ...
                if (repository != null && runId > 0) BuildDetailsScreen(mode, repository, runId, navController::navigateUp,
                ...
                else Column(Modifier.fillMaxSize().padding(24.dp)) { Text("Build unavailable"); Button(onClick = navController::navigateUp) { Text("Back to Builds") } }
            }
            ...
            composable(
                ...
                ProfileScreenWithMetricNavigation(mode, state.profile, state.profileExplorer, onInspectProfile,
                    ...
                    { url -> nativeProfileDestination(url)?.let { openNativeProfile(it.login, it.section) } ?: onOpenGitHubUrl(url) }, openRepo, onLogout,
            ...
            composable(TopDestinationV2.Options.route) { GitHubSettingsScreen(state.profile, { login -> openNativeProfile(login, NativeProfileSection.Repositories) },
            ...
            composable("release/{owner}/{repo}/{tag}", arguments = listOf(navArgument("owner") { type = NavType.StringType }, navArgument("repo") { type = NavType.StringType }, navArgument("tag") { type = NavType.StringType })) { e ->
                RepositoryHubScreen(buildRepositories.firstOrNull { it.owner.login == e.arguments?.getString("owner") && it.name == e.arguments?.getString("repo") }, navController::navigateUp, e.arguments?.getString("tag"))
            }
        }
    }
}
