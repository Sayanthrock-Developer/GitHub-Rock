...
fun MainNavigationV2(
    ...
    val buildRepositories = buildList {
        ...
        ownRepositories.forEach { repo ->
            if (none { it.id == repo.id }) add(repo)
        }
    }
    ...
    val selectBuildRepository: (com.sayanthrock.githubrock.core.model.GitHubRepositoryModel) -> Unit = { repo ->
        onRememberRepository(repo)
    }
    ...
    Box(Modifier.fillMaxSize()) {
        NavHost(navController, TopDestinationV2.Home.route, Modifier.fillMaxSize().widthIn(max = 1200.dp)) {
            ...
            composable(TopDestinationV2.Explore.route) {
                ExploreScreen(onOpenRepo = openRepo, onOpenProfile = { login ->
            ...
            composable(BUILD_STATUS_ROUTE, arguments = listOf(navArgument("filter") { type = NavType.StringType })) { e ->
                BuildStatusPage(
                    ...
                    openRepo,
                    { repo, run -> navController.navigate("build-details/${repo.owner.login}/${repo.name}/${run.id}") },
                    state.profile?.login
                )
            }
            composable(
                ...
                val owner = e.arguments?.getString("owner").orEmpty()
                val repo = e.arguments?.getString("repo").orEmpty()
                val runId = e.arguments?.getLong("runId") ?: 0L
                val repository = buildRepositories.firstOrNull {
                    it.owner.login.equals(owner, true) && it.name == repo
                }
                if (repository != null && runId > 0) {
                    BuildDetailsScreen(
                        ...
                        { id, jobId -> navController.navigate("build-details/$owner/$repo/$id/job/$jobId") },
                        { id, artifactId -> navController.navigate("build-details/$owner/$repo/$id/artifact/$artifactId") }
                    )
                } else {
            ...
            composable(
                ...
                val repo = e.arguments?.getString("repo").orEmpty()
                val runId = e.arguments?.getLong("runId") ?: 0L
                val jobId = e.arguments?.getLong("jobId") ?: 0L
                buildRepositories.firstOrNull {
                    it.owner.login.equals(owner, true) && it.name == repo
                }?.let { BuildJobDetailsScreen(mode, it, runId, jobId, navController::navigateUp) }
                    ?: Text("Repository unavailable")
            }
            composable(
                BUILD_ARTIFACT_DETAILS_ROUTE,
                arguments = listOf(
                    navArgument("owner") { type = NavType.StringType },
                    navArgument("repo") { type = NavType.StringType },
                    navArgument("runId") { type = NavType.LongType },
                    navArgument("artifactId") { type = NavType.LongType }
                )
            ) { e ->
                val owner = e.arguments?.getString("owner").orEmpty()
                val repo = e.arguments?.getString("repo").orEmpty()
                val runId = e.arguments?.getLong("runId") ?: 0L
                val artifactId = e.arguments?.getLong("artifactId") ?: 0L
                buildRepositories.firstOrNull {
                    it.owner.login.equals(owner, true) && it.name == repo
                }?.let { BuildArtifactDetailsScreen(it, runId, artifactId, navController::navigateUp) }
                    ?: Text("Repository unavailable")
            }
            ...
            composable(TopDestinationV2.Profile.route) {
                ProfileScreenWithMetricNavigation(
                    mode,
                    state.profile,
                    state.profileExplorer,
                    onInspectProfile,
                    { navController.navigate(TopDestinationV2.Downloads.route) { launchSingleTop = true } },
                    { navController.navigate(FEATURES_PREVIEW_ROUTE) },
                    { navController.navigate(ACCOUNT_SWITCHER_ROUTE) },
                    openSettings,
                    { navController.navigate(APP_INFORMATION_ROUTE) },
                    { url -> nativeProfileDestination(url)?.let { openNativeProfile(it.login, it.section) } ?: onOpenGitHubUrl(url) },
                    openRepo,
                    onLogout,
                    { state.profile?.login?.let { openNativeProfile(it, NativeProfileSection.Repositories) } },
                    { state.profile?.login?.let { openNativeProfile(it, NativeProfileSection.Followers) } },
                    { state.profile?.login?.let { openNativeProfile(it, NativeProfileSection.Following) } }
                )
            }
            composable(TopDestinationV2.Options.route) {
                GitHubSettingsScreen(
                    state.profile,
                    { login -> openNativeProfile(login, NativeProfileSection.Repositories) },
                    { navController.navigate(ACCOUNT_SWITCHER_ROUTE) { launchSingleTop = true } },
                    { navController.navigate(APP_CUSTOMIZATION_ROUTE) { launchSingleTop = true } },
                    { navController.navigate(TopDestinationV2.Downloads.route) { launchSingleTop = true } },
                    { navController.navigate(APP_INFORMATION_ROUTE) { launchSingleTop = true } },
                    onOpenGitHubUrl,
                    navController::navigateUp
                )
            }
            composable(ACCOUNT_SWITCHER_ROUTE) {
                AccountSwitcherScreen(mode, state.profile, navController::navigateUp, openAccountProfile, onLogout, onLogout, onOpenGitHubUrl)
            }
            composable(
                NATIVE_PROFILE_ROUTE,
                arguments = listOf(
            ...
            composable(
                ...
                BuildsScreen(
                    ...
                    selectBuildRepository,
                    onOpenRepository = openRepo,
                    initialRepository = buildRepositories.firstOrNull {
                        it.owner.login == e.arguments?.getString("owner") && it.name == e.arguments?.getString("repo")
                    },
                    initialRunId = e.arguments?.getLong("runId")
                )
            }
            composable(
