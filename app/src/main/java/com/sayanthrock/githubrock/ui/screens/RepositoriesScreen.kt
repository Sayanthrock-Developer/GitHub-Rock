...
fun RepositoriesScreen(
    ...
    val visibleRepositories = remember(repositories, options) { options.applyLocally(repositories) }
                ...
                DropdownMenu(expanded = sourceMenu, onDismissRequest = { sourceMenu = false }) {
                    RepositorySourceFilter.entries.forEach { item ->
                        DropdownMenuItem(
                            ...
                            onClick = {
                                source = item
                                if (item == RepositorySourceFilter.ConnectedAccount) sourceOwner = connectedLogin.orEmpty()
                                sourceMenu = false
                            }
                        )
                    }
