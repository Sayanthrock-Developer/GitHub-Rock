...
@Composable
fun ApplicationBlurEditor(settings: ApplicationBlurSettings, onMode: (ApplicationBlurMode) -> Unit, onPreset: (ApplicationBlurPreset) -> Unit, onProfile: (ApplicationBlurProfile) -> Unit, onCustomTint: (String) -> Unit = {}, onComponentProfile: (ApplicationBlurComponent, ApplicationBlurProfile?) -> Unit = { _, _ -> }, onReset: () -> Unit = {}) {
    ...
    GlassCard {
        Column(Modifier.fillMaxWidth().padding(4.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            Text("Real app surfaces can use these settings while repository content, text, icons, Markdown, and code remain sharp.")
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) { Text(if (global.enabled) "Enabled" else "Off"); Switch(checked = global.enabled, onCheckedChange = { onProfile(global.copy(enabled = it)) }) }
            Text("Mode"); ChipRow(ApplicationBlurMode.entries, settings.mode, onMode)
            Text("Preset"); ChipRow(ApplicationBlurPreset.entries, settings.preset, onPreset)
            Text("Global profile")
            ProfileControls(selectedProfile) { updated -> if (selectedComponent == null) onProfile(updated) else onComponentProfile(selectedComponent!!, updated) }
            Text("Tint color")
            TextField(value = settings.customTintHex, onValueChange = onCustomTint, modifier = Modifier.fillMaxWidth(), singleLine = true, label = { Text("Custom tint HEX") }, placeholder = { Text("#RRGGBB") })
            ...
            Row(Modifier.fillMaxWidth().horizontalScroll(rememberScrollState()), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                FilterChip(selected = selectedComponent == null, onClick = { selectedComponent = null }, label = { Text("Global") })
                ApplicationBlurComponent.entries.forEach { component -> FilterChip(selected = selectedComponent == component, onClick = { selectedComponent = component }, label = { Text(component.displayName()) }) }
            }
            selectedComponent?.let { component ->
                ...
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    FilterChip(selected = !hasOverride, onClick = { onComponentProfile(component, null) }, label = { Text("Use global") })
                    FilterChip(selected = hasOverride, onClick = { if (!hasOverride) onComponentProfile(component, settings.profileFor(component)) }, label = { Text("Custom") })
                }
                ProfileControls(selectedProfile) { onComponentProfile(component, it) }
            }
            Spacer(Modifier.height(4.dp))
            OutlinedButton(onClick = onReset, modifier = Modifier.fillMaxWidth()) { Text("Reset Application Blur") }
...
@Composable
private fun ProfileControls(profile: ApplicationBlurProfile, onProfile: (ApplicationBlurProfile) -> Unit) {
    ...
    Text("Border"); ChipRow(ApplicationBlurBorder.entries, profile.border) { onProfile(profile.copy(border = it)) }
    BlurSlider("Border opacity", profile.borderOpacity, 0..100) { onProfile(profile.copy(borderOpacity = it)) }
    Text("Shadow"); ChipRow(ApplicationBlurShadow.entries, profile.shadow) { onProfile(profile.copy(shadow = it)) }
    Text("Tint"); ChipRow(ApplicationBlurTint.entries, profile.tint) { onProfile(profile.copy(tint = it)) }
...
private fun <T> ChipRow(values: List<T>, selected: T, onSelected: (T) -> Unit) {
    Row(Modifier.fillMaxWidth().horizontalScroll(rememberScrollState()), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        values.forEach { value -> FilterChip(selected = value == selected, onClick = { onSelected(value) }, label = { Text(value.displayName()) }) }
    }
}
