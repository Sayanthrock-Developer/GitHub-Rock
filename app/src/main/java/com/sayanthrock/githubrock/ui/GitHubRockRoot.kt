...
fun GitHubRockRoot(viewModel: MainViewModel = hiltViewModel(), appearanceViewModel: AppearanceViewModel = hiltViewModel(), setupViewModel: GitHubRockSetupViewModel = hiltViewModel()) {
    ...
    Box(Modifier.fillMaxSize().background(MaterialTheme.colorScheme.background).windowInsetsPadding(WindowInsets.statusBars)) {
        if (state.mode == null) {
            LoginScreenV2(configured = viewModel.loginConfigured, loading = state.isLoading, auth = state.auth, onLogin = viewModel::startDeviceCodeLogin, onDeviceCodeLogin = viewModel::startDeviceCodeLogin, onOpenGitHubUrl = openGitHubUrl, onCheckAuthorization = viewModel::checkLoginStatus, onGuest = viewModel::continueAsGuest)
        } else {
